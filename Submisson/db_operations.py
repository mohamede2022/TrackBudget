...
class UserDAO:
    ...
    def update_user_financials(self, user_id, initial_balance, upcoming_bills):
        """Updates the user's base balance and upcoming bills."""
        with self.db as connection:
            if not connection:
                return False
            ...
            try:
                ...
                query = """
                    UPDATE users
                    SET initial_balance = %s, upcoming_bills = %s
                    WHERE user_id = %s
                """
                cursor.execute(query, (initial_balance, upcoming_bills, user_id))
                connection.commit()
                return True
            except Exception as e:
                print(f"Failed to update financials: {e}")
                return False
