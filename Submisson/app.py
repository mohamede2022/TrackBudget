...
def update_settings():
    ...
    upcoming_bills = request.form.get('upcoming_bills')
    ...
    if initial_balance is not None and upcoming_bills is not None:
        user_dao.update_user_financials(user_id, float(initial_balance), float(upcoming_bills))
...
def dashboard():
    ...
    monthly_spending = sum(
        ...
        if t['money_spent'] 
        and t['date_of_transaction'].month == now.month - 1
        and t['date_of_transaction'].year == now.year
    )
    
    initial_balance = float(user.get('initial_balance') or 0.00)
    upcoming_bills = float(user.get('upcoming_bills') or 0.00)
    
    total_balance = initial_balance - monthly_spending 
...
def send_message():
    ...
    monthly_spending = sum(
        ...
        if t['money_spent'] and t['date_of_transaction'].month == now.month - 1 and t['date_of_transaction'].year == now.year
    )
...
if __name__ == '__main__':
    app.run(host="0.0.0.0", port=443, debug=False, ssl_context=("track.budget.pem", "track.budget-key.pem"))
